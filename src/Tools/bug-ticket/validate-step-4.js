...
export default {
    ...
    methods: {
        ...
        checkScreenshotLinkList() {
            ...
            if (this.screenshots.length < 1 && !['frameready','tritech','dive360'].includes(this.brand)) {
                error_array.push(`Screenshot list is empty. Please make sure that the list has at least one screenshot link provided.`);
                return error_array;
            }
            //if list is longer than 0, loop through each link in the list with the below function, this checks that there isn't more than one link in a row. If a duplicate link is found in the row then it is removed, if the second link found isn't a duplicate an error is thrown to the user to make sure they delete any extra data out of the row
            this.screenshots.forEach((screenshot, index) => {
                //set current loop row value to be called on
                if (screenshot !== '') {//if there is data in the row, check how many links are in it
                    let row_data = screenshot.match(/https?/g) || [];
                    ...
                    if (row_data.length > 1) {
                        ...
                        let dup_link_check = {//object created to find duplicate links
                            times_iterated: 0,
                            iterator: screenshot.matchAll(/https?/g),
                            _constructor: () => {
        ...
        checkVideoLinkList() {
            let error_array = [];
            //check that there are rows to look through
            if (this.videos.length < 1 && !['frameready','tritech','dive360'].includes(this.brand)) {
                error_array.push(`Video list is empty. Please make sure that the list has at least one video link provided.`);
                return error_array;
            }
            //if list is longer than 0, loop through each link in the list with the below function, this checks that there isn't more than one link in a row. If a duplicate link is found in the row then it is removed, if the second link found isn't a duplicate an error is thrown to the user to make sure they delete any extra data out of the row
            this.videos.forEach((video, index) => {
                //set current loop row value to be called on
                if (video !== '') {//if there is data in the row, check how many links are in it
                    let row_data = video.match(/https?/g) || [];
                    //if the array returned is longer than 1 then there is a duplicate
                    if (row_data.length > 1) {
                        //let's find where second link starts in the string
                        let dup_link_check = {//object created to find duplicate links
                            times_iterated: 0,
                            iterator: video.matchAll(/https?/g),
                            _constructor: () => {
                                for (let match of dup_link_check.iterator) {
                                    dup_link_check.times_iterated ++;
                                    dup_link_check[`match_${dup_link_check.times_iterated}`] = match.index;
                        ...
                        dup_link_check._constructor();
                        //find part of url string that is the first link
                        let first_link = video.substr(0, dup_link_check.match_2);
                        //check full string to see if the first link is found more than one time in fullness
                        if (video.match(this.urlRegExBuilder(first_link)).length > 1) {
                            //if first link string is seen more than one time, set string value of the element to be just the first link
                            this.videos[index] = first_link;
                        } else {
                            //otherwise set return value as true for a potentially issue
                            error_array.push(`Video ${index + 1} has more than one link.`);
                        }
                        //check if the link has bad characters in it
                        if (video.includes('(') || video.includes(')')) {
                            error_array.push(`Video ${index + 1}: please remove any parenthesis from the link.`);
                        }
                    }
                }
            });
