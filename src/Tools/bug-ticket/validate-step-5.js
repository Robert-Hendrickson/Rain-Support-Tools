    ...
    mounted() {
        try {
            if (location.search === '?test' || location.hostname === 'localhost') {
                this.examplesValue = 'https://www.google.com';
                this.errorsValue = 'Error: Test error';
            }
        } catch(error) {
            console.error(`Error setting test data for validate-step-5`, error);
        }
    },
    methods: {
        ...
        async validate(returnData){
            let bad_data_list = {};
            //check example data
            if(regexController.regexPatterns.admin_domains.test(this.examplesValue)){
            ...
            if(RegExp(/^[nN](?:\\|\/)?[aA]/).test(this.examplesValue) || this.examplesValue === ''){
                bad_data_list['examples_blank'] = 'Examples cannot be blank or say n/a.';
            }
            if (regexController.regexPatterns.salesforce.test(this.examplesValue)) {
            ...
            //check errors data
            if(RegExp(/^[nN](?:\\|\/)?[aA]/).test(this.errorsValue) || this.errorsValue === ''){
            ...
            if(RegExp(/^(see|look at).*(video|screenshot(s)?)/i).test(this.errorsValue)){
        ...
        async checkDescriptionNeedsLinkExamples(){
            ...
            let description_check_array = this.description.match(/website|cart|checkout|add to cart/gi) || [];
            //check if there is a link in the examples data
            let example_check_for_links = (/(?:https?:\/\/)?(?:\w+\.)?(\w+\.)+\w{3,}/g).test(this.examplesValue);
            let check = true;
        ...
        async checkUnnecessaryErrors(){
            let check = true;
            let bad_error_array = [];
            if ((/Blocked aria-hidden/i).test(this.errorsValue)) {
