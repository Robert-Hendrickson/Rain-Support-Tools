...
function addRowElement(){
    ...
    setAttributes(in_qty, {
        'type': 'number',
        'id': 'qty',
        'value': '0',
        'onchange': `lineUpdate(${line})`
    });
    ...
    setAttributes(mat_taxable, {
        'type': 'checkbox',
        'id': `row_${number_of_lines}_mat`,
        'onChange': `booleanUpdate(${line},'mat')`
    });
    td_taxable.appendChild(mat_taxable);
    const serv_taxable = $(document)[0].createElement("input");
    setAttributes(serv_taxable, {
        'type': 'checkbox',
        'id': `row_${number_of_lines}_serv`,
        'onChange': `booleanUpdate(${line},'serv')`
    });
    td_taxable.appendChild(serv_taxable);
    const class_taxable = $(document)[0].createElement("input");
    setAttributes(class_taxable, {
        'type': 'checkbox',
        'id': `row_${number_of_lines}_class`,
        'onChange': `booleanUpdate(${line},'class')`
    });
